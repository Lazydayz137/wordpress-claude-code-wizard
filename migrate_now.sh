...
# Check for specific SSH key, otherwise default to system SSH agent
if [ -f "$HOME/.ssh/wordpress_deploy" ]; then
    SSH_OPT="-i $HOME/.ssh/wordpress_deploy"
else
    SSH_OPT=""
fi
...
scp $SSH_OPT -o StrictHostKeyChecking=no wp-content.tar.gz root@$DROPLET_IP:/tmp/
scp $SSH_OPT -o StrictHostKeyChecking=no wordpress_backup.sql root@$DROPLET_IP:/tmp/

# 5. Install on droplet
echo "5. Installing on droplet..."
ssh $SSH_OPT -o StrictHostKeyChecking=no root@$DROPLET_IP << ENDSSH
# Backup existing
cd /var/www/html
